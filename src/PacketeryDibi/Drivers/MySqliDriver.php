 ...
 */
class MySqliDriver implements PacketeryDibi\Driver, PacketeryDibi\ResultDriver
{
	...
	public function __construct()
	{
		if (!extension_loaded('mysqli')) {
			throw new PacketeryDibi\NotSupportedException("PHP extension 'mysqli' is not loaded.");
		}
	}
	...
	public function connect(array &$config)
	{
		...
		if (isset($config['resource'])) {
			...
			if ($errno = mysqli_connect_errno()) {
				throw new PacketeryDibi\DriverException(mysqli_connect_error(), $errno);
			}
		}
	...
	public function getInfo()
	{
		...
		if (preg_last_error()) {
			throw new PacketeryDibi\PcreException;
		}
	...
	public function applyLimit(&$sql, $limit, $offset)
	{
		if ($limit < 0 || $offset < 0) {
			...
			$sql .= ' LIMIT ' . ($limit === null ? '18446744073709551615' : PacketeryDibi\Helpers::intVal($limit))
				. ($offset ? ' OFFSET ' . PacketeryDibi\Helpers::intVal($offset) : '');
		}
	}
	...
	public function getResultColumns()
	{
		...
		for ($i = 0; $i < $count; $i++) {
			...
			$columns[] = [
				...
				'type' => $row['type'] === MYSQLI_TYPE_TIME ? PacketeryDibi\Type::TIME_INTERVAL : null,
				'vendor' => $row,
			];
		}
		return $columns;
