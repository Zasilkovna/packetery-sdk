 ...
 */
class MySqlDriver implements PacketeryDibi\Driver, PacketeryDibi\ResultDriver
{
	...
	public function __construct()
	{
		if (!extension_loaded('mysql')) {
			throw new PacketeryDibi\NotSupportedException("PHP extension 'mysql' is not loaded.");
		}
	}
	...
	public function connect(array &$config)
	{
		...
		if (!is_resource($this->connection)) {
			throw new PacketeryDibi\DriverException(mysql_error(), mysql_errno());
		}
		...
		if (isset($config['database'])) {
			if (!@mysql_select_db($config['database'], $this->connection)) { // intentionally @
				throw new PacketeryDibi\DriverException(mysql_error($this->connection), mysql_errno($this->connection));
			}
		}
	...
	public function getResultColumns()
	{
		...
		for ($i = 0; $i < $count; $i++) {
			...
			$columns[] = [
				...
				'type' => $row['type'] === 'time' ? PacketeryDibi\Type::TIME_INTERVAL : null,
				'vendor' => $row,
			];
		}
		return $columns;
