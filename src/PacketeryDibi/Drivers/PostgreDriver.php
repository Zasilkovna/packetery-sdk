 ...
 */
class PostgreDriver implements PacketeryDibi\Driver, PacketeryDibi\ResultDriver, PacketeryDibi\Reflector
{
	...
	public function __construct()
	{
		if (!extension_loaded('pgsql')) {
			throw new PacketeryDibi\NotSupportedException("PHP extension 'pgsql' is not loaded.");
		}
	}
	...
	public function connect(array &$config)
	{
		...
		if (!is_resource($this->connection)) {
			throw new PacketeryDibi\DriverException($error ?: 'Connecting error.');
		}
	...
	{
		if (!is_resource($this->connection)) {
			throw new PacketeryDibi\Exception('Lost connection to server.');
		}
	...
	public function applyLimit(&$sql, $limit, $offset)
	{
		...
		if ($limit !== null) {
			$sql .= ' LIMIT ' . PacketeryDibi\Helpers::intVal($limit);
		}
		if ($offset) {
			$sql .= ' OFFSET ' . PacketeryDibi\Helpers::intVal($offset);
		}
	}
	...
	public function getTables()
	{
		...
		if ($version < 7.4) {
			throw new PacketeryDibi\DriverException('Reflection requires PostgreSQL 7.4 and newer.');
		}
