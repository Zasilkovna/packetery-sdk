 ...
 */
class OdbcDriver implements PacketeryDibi\Driver, PacketeryDibi\ResultDriver, PacketeryDibi\Reflector
{
	...
	public function __construct()
	{
		if (!extension_loaded('odbc')) {
			throw new PacketeryDibi\NotSupportedException("PHP extension 'odbc' is not loaded.");
		}
	}
	...
	public function connect(array &$config)
	{
		...
		if (!is_resource($this->connection)) {
			throw new PacketeryDibi\DriverException(odbc_errormsg() . ' ' . odbc_error());
		}
	...
	public function query($sql)
	{
		...
		if ($res === false) {
			throw new PacketeryDibi\DriverException(odbc_errormsg($this->connection) . ' ' . odbc_error($this->connection), 0, $sql);
	...
	{
		throw new PacketeryDibi\NotSupportedException('ODBC does not support autoincrementing.');
	}
	...
	public function begin($savepoint = null)
	{
		if (!odbc_autocommit($this->connection, false)) {
			throw new PacketeryDibi\DriverException(odbc_errormsg($this->connection) . ' ' . odbc_error($this->connection));
		}
	}
