 ...
 */
class MsSqlDriver implements PacketeryDibi\Driver, PacketeryDibi\ResultDriver
{
	...
	public function __construct()
	{
		if (!extension_loaded('mssql')) {
			throw new PacketeryDibi\NotSupportedException("PHP extension 'mssql' is not loaded.");
		}
	}
	...
	public function connect(array &$config)
	{
		...
		if (!is_resource($this->connection)) {
			throw new PacketeryDibi\DriverException("Can't connect to DB.");
		}

		if (isset($config['database']) && !@mssql_select_db($this->escapeIdentifier($config['database']), $this->connection)) { // intentionally @
			throw new PacketeryDibi\DriverException("Can't select DB '$config[database]'.");
		}
	}
	...
	public function query($sql)
	{
		...
		if ($res === false) {
			throw new PacketeryDibi\DriverException(mssql_get_last_message(), 0, $sql);
