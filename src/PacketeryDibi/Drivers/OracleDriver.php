 ...
 */
class OracleDriver implements PacketeryDibi\Driver, PacketeryDibi\ResultDriver, PacketeryDibi\Reflector
{
	...
	public function __construct()
	{
		if (!extension_loaded('oci8')) {
			throw new PacketeryDibi\NotSupportedException("PHP extension 'oci8' is not loaded.");
		}
	}
	...
	public function connect(array &$config)
	{
		...
		if (!$this->connection) {
			$err = oci_error();
			throw new PacketeryDibi\DriverException($err['message'], $err['code']);
		}
	...
	public function query($sql)
	{
		...
		if ($res) {
			...
			throw new PacketeryDibi\DriverException($err['message'], $err['code'], $sql);
		}
		return null;
	}
	...
	/**
	 * @return PacketeryDibi\DriverException
	 */
	public static function createException($message, $code, $sql)
	{
		if (in_array($code, [1, 2299, 38911], true)) {
			return new PacketeryDibi\UniqueConstraintViolationException($message, $code, $sql);

		} elseif (in_array($code, [1400], true)) {
			return new PacketeryDibi\NotNullConstraintViolationException($message, $code, $sql);

		} elseif (in_array($code, [2266, 2291, 2292], true)) {
			return new PacketeryDibi\ForeignKeyConstraintViolationException($message, $code, $sql);

		} else {
			return new PacketeryDibi\DriverException($message, $code, $sql);
		}
	}
	...
	public function getInsertId($sequence)
	{
		...
		return isset($row['ID']) ? PacketeryDibi\Helpers::intVal($row['ID']) : false;
	}
	...
	public function applyLimit(&$sql, $limit, $offset)
	{
		if ($limit < 0 || $offset < 0) {
			...
			$sql = 'SELECT * FROM (' . $sql . ') WHERE ROWNUM <= ' . PacketeryDibi\Helpers::intVal($limit);
		}
	}
	...
	{
		throw new PacketeryDibi\NotSupportedException('Row count is not available for unbuffered queries.');
	}
