...
namespace PacketeryDibi\Drivers;
 ...
 */
class FirebirdDriver implements PacketeryDibi\Driver, PacketeryDibi\ResultDriver, PacketeryDibi\Reflector
{
	...
	public function __construct()
	{
		if (!extension_loaded('interbase')) {
			throw new PacketeryDibi\NotSupportedException("PHP extension 'interbase' is not loaded.");
		}
	}
	...
	public function connect(array &$config)
	{
		...
		if (isset($config['resource'])) {
			...
			if (!is_resource($this->connection)) {
				throw new PacketeryDibi\DriverException(ibase_errmsg(), ibase_errcode());
			}
		}
	}
	 ...
	 * @throws PacketeryDibi\DriverException|Dibi\Exception
	 */
	public function query($sql)
	{
		...
		if ($res === false) {
			if (ibase_errcode() == self::ERROR_EXCEPTION_THROWN) {
				...
				throw new PacketeryDibi\ProcedureException($match[3], $match[1], $match[2], $sql);

			} else {
				throw new PacketeryDibi\DriverException(ibase_errmsg(), ibase_errcode(), $sql);
			}
	...
	{
		if ($savepoint !== null) {
			throw new PacketeryDibi\NotSupportedException('Savepoints are not supported in Firebird/Interbase.');
		}
	...
	public function commit($savepoint = null)
	{
		...
		if (!ibase_commit($this->transaction)) {
			throw new PacketeryDibi\DriverException('Unable to handle operation - failure when commiting transaction.');
		}
	...
	public function rollback($savepoint = null)
	{
		...
		if (!ibase_rollback($this->transaction)) {
			throw new PacketeryDibi\DriverException('Unable to handle operation - failure when rolbacking transaction.');
		}
	...
	public function applyLimit(&$sql, $limit, $offset)
	{
		if ($limit > 0 || $offset > 0) {
			...
			$sql = 'SELECT ' . ($limit > 0 ? 'FIRST ' . PacketeryDibi\Helpers::intVal($limit) : '')
				. ($offset > 0 ? ' SKIP ' . PacketeryDibi\Helpers::intVal($offset) : '')
				. ' * FROM (' . $sql . ')';
		}
	}
	...
	{
		throw new PacketeryDibi\NotSupportedException('Firebird/Interbase do not support returning number of rows in result set.');
	}
	...
	public function fetch($assoc)
	{
		...
		if (ibase_errcode()) {
			if (ibase_errcode() == self::ERROR_EXCEPTION_THROWN) {
				...
				throw new PacketeryDibi\ProcedureException($match[3], $match[1], $match[2]);
	...
	{
		throw new PacketeryDibi\NotSupportedException('Firebird/Interbase do not support seek in result set.');
	}
